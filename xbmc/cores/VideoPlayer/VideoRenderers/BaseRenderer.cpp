...
RESOLUTION CBaseRenderer::FindClosestResolution(float fps, float multiplier, RESOLUTION current, float& weight)
{
  ...
  float curr_diff = std::abs(m_sourceWidth - curr.iScreenWidth);
  float loop_diff = 0.0f;
  ...
  for (size_t i = (int)RES_DESKTOP; i < CDisplaySettings::GetInstance().ResolutionInfoSize(); i++)
  {
    ...
    ||  info.fRefreshRate < (fRefreshRate * multiplier / 1.001) - 0.001)
    {
      ...
      if ((m_sourceWidth < orig.iScreenWidth) // orig res large enough
    ...
    loop_diff = std::abs(m_sourceWidth - info.iScreenWidth);
    curr_diff = std::abs(m_sourceWidth - curr.iScreenWidth);
