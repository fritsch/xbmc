...
static bool ResolveChannelLayout(pa_channel_map& PAchmap, AEAudioFormat &format)
{
  CAEChannelInfo AEchmap;
  pa_channel_map_init(&PAchmap);
  if (!AE_IS_RAW(format.m_dataFormat))
  {
    unsigned int wantedChannels = format.m_channelLayout.Count();
 
    for (unsigned int AEch = 0; AEch < wantedChannels && PAchmap.channels <= PA_CHANNELS_MAX; ++AEch)
    {
      bool match = true;
      switch (format.m_channelLayout[AEch])
      {
        case AE_CH_FL:    PAchmap.map[PAchmap.channels] = PA_CHANNEL_POSITION_FRONT_LEFT; break;
        case AE_CH_FR:    PAchmap.map[PAchmap.channels] = PA_CHANNEL_POSITION_FRONT_RIGHT; break;
        case AE_CH_FC:    PAchmap.map[PAchmap.channels] = PA_CHANNEL_POSITION_FRONT_CENTER; break;
        case AE_CH_LFE:   PAchmap.map[PAchmap.channels] = PA_CHANNEL_POSITION_LFE; break;
        case AE_CH_BL:    PAchmap.map[PAchmap.channels] = PA_CHANNEL_POSITION_REAR_LEFT; break;
        case AE_CH_BR:    PAchmap.map[PAchmap.channels] = PA_CHANNEL_POSITION_REAR_RIGHT; break;
        case AE_CH_FLOC:  PAchmap.map[PAchmap.channels] = PA_CHANNEL_POSITION_FRONT_LEFT_OF_CENTER; break;
        case AE_CH_FROC:  PAchmap.map[PAchmap.channels] = PA_CHANNEL_POSITION_FRONT_RIGHT_OF_CENTER; break;
        case AE_CH_BC:    PAchmap.map[PAchmap.channels] = PA_CHANNEL_POSITION_REAR_CENTER; break;
        case AE_CH_SL:    PAchmap.map[PAchmap.channels] = PA_CHANNEL_POSITION_SIDE_LEFT; break;
        case AE_CH_SR:    PAchmap.map[PAchmap.channels] = PA_CHANNEL_POSITION_SIDE_RIGHT; break;
        case AE_CH_TFL:   PAchmap.map[PAchmap.channels] = PA_CHANNEL_POSITION_TOP_FRONT_LEFT; break;
        case AE_CH_TFR:   PAchmap.map[PAchmap.channels] = PA_CHANNEL_POSITION_TOP_FRONT_RIGHT; break;
        case AE_CH_TFC:   PAchmap.map[PAchmap.channels] = PA_CHANNEL_POSITION_TOP_FRONT_CENTER; break;
        case AE_CH_TC:    PAchmap.map[PAchmap.channels] = PA_CHANNEL_POSITION_TOP_CENTER; break;
        case AE_CH_TBL:   PAchmap.map[PAchmap.channels] = PA_CHANNEL_POSITION_TOP_REAR_LEFT; break;
        case AE_CH_TBR:   PAchmap.map[PAchmap.channels] = PA_CHANNEL_POSITION_TOP_REAR_RIGHT; break;
        case AE_CH_TBC:   PAchmap.map[PAchmap.channels] = PA_CHANNEL_POSITION_TOP_REAR_CENTER; break;
        default:
          match = false;
          break;
      }
      ...
      {
        PAchmap.channels++;
        AEchmap += format.m_channelLayout[AEch];
      }
    }
    if (PAchmap.channels == 0)
    {
      CLog::Log(LOGERROR, "CAESinkPulseAudio - unable to enumerate channels");
      return false;
    }
    format.m_channelLayout = AEchmap;
  }
  else 
  {
...
bool CAESinkPULSE::Initialize(AEAudioFormat &format, std::string &device)
{
  ...
  if(!ResolveChannelLayout(map, format))
    return false;
