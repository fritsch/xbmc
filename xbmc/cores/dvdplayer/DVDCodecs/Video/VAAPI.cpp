...
bool CVaapiRenderPicture::CopyGlx()
{
  ...
  if (glxCrap.bound == true)
    return true;

  if (glxCrap.procPic.source == CVaapiProcessedPicture::SKIP_SRC ||
      glxCrap.procPic.source == CVaapiProcessedPicture::VPP_SRC)
  {
    unsigned int colorStandard;
    switch(glxCrap.procPic.DVDPic.color_matrix)
    {
    ...
    if (vaSyncSurface(glxCrap.vadsp, glxCrap.procPic.videoSurface) != VA_STATUS_SUCCESS)
      return false;

    if (vaPutSurface(glxCrap.vadsp,
                     glxCrap.procPic.videoSurface,
                     glxCrap.pixmap,
                     0,0,
                     texWidth, texHeight,
                     0,0,
    ...
    XSync(glxCrap.x11dsp, false);
    glEnable(glxCrap.textureTarget);
    glBindTexture(glxCrap.textureTarget, texture);
    glxCrap.glXBindTexImageEXT(glxCrap.x11dsp, glxCrap.glPixmap, GLX_FRONT_LEFT_EXT, NULL);
    glBindTexture(glxCrap.textureTarget, 0);
    glDisable(glxCrap.textureTarget);

    glxCrap.bound = true;

    vaapi->ReturnProcPicture(glxCrap.procPic.id);
    glxCrap.procPic.id = -1;
  }
...
void CVaapiRenderPicture::Sync()
{
  ...
  if (DVDPic.format == RENDER_FMT_VAAPI && glxCrap.bound)
  {
    glEnable(glxCrap.textureTarget);
    glBindTexture(glxCrap.textureTarget, texture);
    glxCrap.glXReleaseTexImageEXT(glxCrap.x11dsp, glxCrap.glPixmap, GLX_FRONT_LEFT_EXT);
    glBindTexture(glxCrap.textureTarget, 0);
    glDisable(glxCrap.textureTarget);
  }
}
...
CVaapiRenderPicture* COutput::ProcessPicture(CVaapiProcessedPicture &pic)
{
      ...
      pic.source == CVaapiProcessedPicture::VPP_SRC)
  {
    ...
    retPic->glxCrap.procPic = pic;
    retPic->glxCrap.bound = false;
  }
...
void COutput::ProcessReturnPicture(CVaapiRenderPicture *pic)
{
  ...
  ProcessReturnProcPicture(pic->glxCrap.procPic.id);
  pic->valid = false;
}
...
bool COutput::EnsureBufferPool()
{
  ...
  for (unsigned int i = 0; i < m_bufferPool.allRenderPics.size(); i++)
  {
    ...
    pic->glxCrap.pixmap = XCreatePixmap(m_Display,
                                m_Window,
                                m_config.outWidth,
                                m_config.outHeight,
                                wndattribs.depth);
    if (!pic->glxCrap.pixmap)
    {
    ...
    // create gl pixmap
    pic->glxCrap.glPixmap = glXCreatePixmap(m_Display, fbConfigs[fbConfigIndex], pic->glxCrap.pixmap, pixmapAttribs);

    if (!pic->glxCrap.glPixmap)
    {
    ...
    pic->glxCrap.vadsp = m_config.dpy;
    pic->glxCrap.x11dsp = m_Display;
    pic->glxCrap.glXBindTexImageEXT = glXBindTexImageEXT;
    pic->glxCrap.glXReleaseTexImageEXT = glXReleaseTexImageEXT;
    pic->glxCrap.textureTarget = m_textureTarget;
...
void COutput::ReleaseBufferPool(bool precleanup)
{
  ...
  for (unsigned int i = 0; i < m_bufferPool.allRenderPics.size(); i++)
  {
    ...
    if (pic->texture)
    {
      ...
      glXDestroyPixmap(m_Display, pic->glxCrap.glPixmap);
      XFreePixmap(m_Display, pic->glxCrap.pixmap);
      pic->texture = None;
    }
