...
void appendEmptyOptions(DBusMessageIter *argsIter)
{
...
CUDisks2Drive::CUDisks2Drive(const char *object) : m_object(object)
{
}

bool CUDisks2Drive::IsOptical()
{
...
std::string CUDisks2Drive::toString()
{
  return StringUtils::Format("Drive %s: IsRemovable %s IsOptical %s",
                             m_object, BOOL2SZ(m_isRemovable), BOOL2SZ(IsOptical()));
}

CUDisks2Block::CUDisks2Block(const char *object) : m_object(object)
{
}

bool CUDisks2Block::IsReady()
{
  return m_drive != nullptr;
}

bool CUDisks2Filesystem::IsOptical()
{
  return  m_block->m_drive->IsOptical();
}

std::string CUDisks2Block::toString()
{
...
CUDisks2Filesystem::CUDisks2Filesystem(const char *object) : m_object(object)
{
}

std::string CUDisks2Filesystem::toString()
{
...
bool CUDisks2Filesystem::IsReady()
{
  return m_block != nullptr && m_block->IsReady();
}

bool CUDisks2Filesystem::Mount()
{
  if (!m_isMounted && !m_block->m_isSystem)
  {
...
bool CUDisks2Filesystem::Unmount()
{
  if (m_isMounted && !m_block->m_isSystem)
  {
  ...
  {
    CLog::Log(LOGDEBUG, "UDisks2: Refuse to unmount %s", toString());
    return false;
  }
}

std::string CUDisks2Filesystem::GetDisplayName()
{
...
CMediaSource CUDisks2Filesystem::ToMediaShare()
{
  CMediaSource source;
...
bool CUDisks2Filesystem::IsApproved()
{
  return IsReady() &&
...
void CUDisks2Provider::Initialize()
{
  ...
  m_daemonVersion = CDBusUtil::GetVariant(UDISKS2_SERVICE_UDISKS2, UDISKS2_PATH_MANAGER, UDISKS2_INTERFACE_MANAGER,
                                          "Version").asString();
  CLog::Log(LOGDEBUG, "UDisks2: DaemonVersion %s", m_daemonVersion);
...
bool CUDisks2Provider::Eject(const std::string &mountpath)
{
  ...
  {
    CUDisks2Filesystem *fs = elt.second;
...
void CUDisks2Provider::DriveAdded(CUDisks2Drive *drive)
{
  CLog::Log(LOGDEBUG, LOGDBUS, "UDisks2: DriveAdded - %s", drive->toString());
...
{
  CLog::Log(LOGDEBUG, LOGDBUS, "UDisks2: DriveRemoved (%s)", object);
...
void CUDisks2Provider::BlockAdded(CUDisks2Block *block, bool isNew)
{
  if (isNew)
  {
    CLog::Log(LOGDEBUG, LOGDBUS, "UDisks2: BlockAdded - %s", block->toString());
...
{
  CLog::Log(LOGDEBUG, LOGDBUS, "UDisks2: BlockRemoved (%s)", object);
...
void CUDisks2Provider::FilesystemAdded(CUDisks2Filesystem *fs, bool isNew)
{
  if (isNew)
  {
    CLog::Log(LOGDEBUG, LOGDBUS, "UDisks2: FilesystemAdded - %s", fs->toString());
...
{
  CLog::Log(LOGDEBUG, LOGDBUS, "UDisks2: FilesystemRemoved (%s)", object);
  bool result = false;
  if (m_filesystems.count(object) > 0)
  {
...
void CUDisks2Provider::ParseInterface(const char *object, const char *iface, DBusMessageIter *propsIter)
{
  if (strcmp(iface, UDISKS2_INTERFACE_DRIVE) == 0)
  {
    auto *drive = new CUDisks2Drive(object);
  ...
  {
    auto *block = new CUDisks2Block(object);
  ...
  {
    auto *fs = new CUDisks2Filesystem(object);
...
void CUDisks2Provider::ParseDriveProperty(CUDisks2Drive *drive, const char *key, DBusMessageIter *varIter)
{
...
void CUDisks2Provider::ParseBlockProperty(CUDisks2Block *block, const char *key, DBusMessageIter *varIter)
{
  switch (dbus_message_iter_get_arg_type(varIter))
  {
...
void CUDisks2Provider::ParseFilesystemProperty(CUDisks2Filesystem *fs, const char *key, DBusMessageIter *varIter)
{
  switch (dbus_message_iter_get_arg_type(varIter))
  {
    case DBUS_TYPE_ARRAY:
