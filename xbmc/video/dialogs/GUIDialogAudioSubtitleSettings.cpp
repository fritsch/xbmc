...
void CGUIDialogAudioSubtitleSettings::OnSettingChanged(const CSetting *setting)
{
  ...
  else if (settingId == SETTING_VIDEO_STREAM)
  {
    ...
    if (g_application.m_pPlayer->GetVideoStream() != m_videoStream)
    {
      ...
      g_application.m_pPlayer->SetVideoStream(m_videoStream);    // Set the audio stream to the one selected
    }
  }
...
void CGUIDialogAudioSubtitleSettings::InitializeSettings()
{
  ...
  AddVideoStreams(groupAudio, SETTING_VIDEO_STREAM);
...
void CGUIDialogAudioSubtitleSettings::AddVideoStreams(CSettingGroup *group, const std::string &settingId)
{
  ...
  AddList(group, settingId, 460, 0, m_videoStream, VideoStreamsOptionFiller, 460);
}
