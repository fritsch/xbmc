...
{
  // Determine cache chunk size: if source chunk size is bigger than 1 use source chunk size else use requested chunk size
...
ssize_t CFile::LoadFile(const CURL& file, auto_buffer& outputBuffer)
{
  ...
  */
  ...
  size_t chunksize = (filesize > 0) ? (size_t)(filesize + 1)
                                    : (size_t)DetermineChunkSize(GetChunkSize(), min_chunk_size);
  size_t total_read = 0;
  while (true)
  {
