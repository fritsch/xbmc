...
int CSimpleFileCache::WriteToCache(const char *pBuffer, size_t iSize)
{
  ...
  while (iSize > 0)
  {
    const ssize_t lastWritten =
        m_cacheFileWrite->Write(pBuffer, std::min(iSize, (size_t)SSIZE_MAX));
    if (lastWritten <= 0)
    {
...
int CSimpleFileCache::ReadFromCache(char *pBuffer, size_t iMaxSize)
{
  ...
  size_t toRead = std::min(iMaxSize, (size_t)iAvailable);

  size_t readBytes = 0;
  while (toRead > 0)
  {
    const ssize_t lastRead = m_cacheFileRead->Read(pBuffer, std::min(toRead, (size_t)SSIZE_MAX));
    if (lastRead == 0)
      break;
    if (lastRead < 0)
    {
